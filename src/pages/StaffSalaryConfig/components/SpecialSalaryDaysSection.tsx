import staffScheduleApis, { ISpecialSalaryDay } from "@/apis/staffSchedule.apis";
...
function SpecialSalaryDaysSection({
  ...
  const onSubmit = (values: SpecialDayFormValues) => {
    ...
    if (Object.keys(hourlyAmountMap).length === 0) {
      toast({
        title: "Thiếu dữ liệu",
        description: "Nhập ít nhất một khung giờ có mức lương ≥ 0 (đủ điều kiện > 0).",
        variant: "destructive",
      });
      return;
    }
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Ghi đè toàn bộ map giờ cho từng ngày kinh doanh (YYYY-MM-DD). Dùng thay
          cho override theo nhân viên.
        </CardDescription>
                    ...
                    <TableCell className="text-right space-x-2">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          if (
                            window.confirm(
                              `Xóa cấu hình ngày ${row.businessDate}?`,
                            )
                          ) {
                            deleteMutation.mutate(row.businessDate);
                          }
                        }}
                      >
